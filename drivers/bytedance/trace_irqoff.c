...
{
	stack_trace_save_skip_hardirq = (void *)kallsyms_lookup_name("stack_trace_save_regs");
}
...
static const struct proc_ops trace_latency_proc_ops = {
...
static const struct proc_ops sampling_period_proc_ops = {
...
static int __init trace_irqoff_init(void)
{
	...
	if (!proc_create("trace_latency", S_IRUSR | S_IWUSR, parent_dir,
			 &trace_latency_proc_ops))
		goto remove_proc;

	if (!proc_create("enable", S_IRUSR | S_IWUSR, parent_dir, &enable_proc_ops))
		goto remove_proc;

	if (!proc_create("sampling_period", S_IRUSR | S_IWUSR, parent_dir,
			 &sampling_period_proc_ops))
		goto remove_proc;
