		...
		unsigned int flags, struct iomap *iomap)
{
	...
	if (map.m_flags & EROFS_MAP_META) {
		...
		ptr = erofs_read_metabuf(&buf, inode->i_sb,
					 erofs_blknr(inode->i_sb, mdev.m_pa), EROFS_KMAP);
		if (IS_ERR(ptr))
			return PTR_ERR(ptr);
		iomap->inline_data = ptr + erofs_blkoff(inode->i_sb, mdev.m_pa);
...
static int erofs_readpages(struct file *filp, struct address_space *mapping,
